...
export function createInventoryContainer(isBordered: boolean): HTMLDivElement {
   const containerElem = document.createElement("div");
   containerElem.classList.add("inventory");
   if (isBordered) {
      containerElem.classList.add("bordered");
   }
   return containerElem;
}

export function createInventory(inventory: Inventory, isBordered: boolean, entity: Entity): HTMLDivElement {
   const containerElem = createInventoryContainer(isBordered);

   // Rows
   let itemSlot = 1;
   for (let i = 0; i < inventory.height; i++) {
      // const rowElem = document.createElement("div");
      // rowElem.classList.add("inventory-row");
      
      // Columns
      for (let j = 0; j < inventory.width; j++) {
         const itemSlotElem = createItemSlot();
         makeItemSlotInteractable(itemSlotElem, entity, inventory, itemSlot);
         // rowElem.appendChild(itemSlotElem);
