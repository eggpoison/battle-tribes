...
import { HealthComponentArray, addLocalInvulnerabilityHash, canDamageEntity, damageEntity, healEntity } from "./HealthComponent.js";
import { ItemComponentArray } from "./ItemComponent.js";
import { TribeComponentArray } from "./TribeComponent.js";
import { createEntity, destroyEntity, entityExists, getEntityLayer, getEntityType, surfaceLayer } from "../world.js";
...
function onTick(yeti: Entity): void {
   ...
   // Eat raw beef
   {
      ...
      for (let i = 0; i < aiHelperComponent.visibleEntities.length; i++) {
         ...
         if (itemComponent.item.type === ItemType.raw_beef || itemComponent.item.type === ItemType.raw_fish) {
            const entityTransformComponent = TransformComponentArray.getComponent(entity);
