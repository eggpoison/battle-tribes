...
{#if playerInstance !== null && HealthComponentArray.hasComponent(playerInstance)}
   {#key healthBarState.health}
      <div id="health-bar" class="animated" style:--current-health={healthBarState.health.toString()} style:--previous-health={healthBarState.previousHealth.toString()}>
         <div class="health-icon">
            <img src={HealthIcon} alt="" />
            <div class="health-counter">{Math.round((healthBarState.health + Number.EPSILON) * 100) / 100}</div>
         </div>
