...
export function sendOpenEntityInventoryPacket(entity: Entity): void {
   const packet = new Packet(PacketType.openEntityInventory, Float32Array.BYTES_PER_ELEMENT);
   packet.writeNumber(entity);
   sendPacket(packet);
}

export function sendCloseEntityInventoryPacket(entity: Entity): void {
   const packet = new Packet(PacketType.closeEntityInventory, Float32Array.BYTES_PER_ELEMENT);
   packet.writeNumber(entity);
   sendPacket(packet);
}