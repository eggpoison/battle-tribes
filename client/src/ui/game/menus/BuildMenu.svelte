<script lang="ts">
   import { BlueprintType, BuildingMaterial } from "webgl-test-shared/src/components";
   import { Entity, EntityType, EntityTypeString } from "webgl-test-shared/src/entities";
   ...
   import { InventoryName, ITEM_TYPE_RECORD, ItemType } from "webgl-test-shared/src/items/items";
   ...
   import WoodenDoorImage from "../../images/entities/door/wooden-door.png";
    import BuildMenuOption from "./BuildMenuOption.svelte";
...
{#if entityExists(buildingID)}
   <div ref={blueprintRef} id="blueprint-menu" onMouseDown={() => click(buildingID, options)} onMouseMove={e => mouseMove(e.nativeEvent)}onMouseLeave={() => {setHoveredOptionIdx(null); clearHoveredGhostType()}}  style={{"--x": x.toString(), "--y": y.toString()} as React.CSSProperties} onContextMenu={e => { e.preventDefault() }}>
