...
import { damageEntity, healEntity, HealthComponentArray, hitEntityWithoutDamage } from "./HealthComponent.js";
...
import { createEntity, destroyEntity, entityExists, getEntityAgeTicks, getEntityLayer, getEntityType, getGameTicks } from "../world.js";
import { getEntitiesAtPosition } from "../layer-utils.js";
import { dismountMount, getAvailableCarrySlot, mountCarrySlot, RideableComponentArray } from "./RideableComponent.js";
import { addSkillLearningProgress, getRiderTargetPosition, TamingComponentArray } from "./TamingComponent.js";
import { CollisionVars, entitiesAreColliding } from "../collision-detection.js";
import { addHitboxVelocity, applyKnockback, getHitboxVelocity, Hitbox } from "../hitboxes.js";
...
function onTick(cow: Entity): void {
      ...
      if (entityHasPassedPosition(cow, tamingComponent.depositTarget) && getDistanceFromPointToEntity(tamingComponent.depositTarget.x, tamingComponent.depositTarget.y, transformComponent) < 100) {
         ...
         for (const carrySlot of rideableComponent.carrySlots) {
            if (entityExists(carrySlot.occupiedEntity)) {
               carriedEntity = carrySlot.occupiedEntity;
               break;
            }
         }
         
         if (carriedEntity !== null) {
            dismountMount(carriedEntity, cow);
            tamingComponent.depositTarget = null;
         }
      }
   ...
   // @SQUEAM for shot where i dont want them to move
