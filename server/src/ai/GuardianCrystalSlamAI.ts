...
import { moveEntityToEntity, stopEntity, turnEntityToEntity } from "../ai-shared";
...
const enum Vars {
   WINDUP_TIME_TICKS = (1.2 * Settings.TPS) | 0,
...
export default class GuardianCrystalSlamAI {
   ...
   private slam(guardian: EntityID): void {
      ...
      for (let offsetIdx = 0; offsetIdx <= 10; offsetIdx++) {
         ...
         for (let i = 0; i < numQuakes; i++) {
            ...
            const spawnDelayTicks = Math.round(offsetIdx * 0.085 * Settings.TPS);
   ...
   public run(guardian: EntityID, target: EntityID): void {
      ...
      if (this.windupProgressTicks < Vars.WINDUP_TIME_TICKS) {
         ...
         guardianComponent.setLimbGemActivations(progress, 0, 0);
         ...
         guardianComponent.setLimbGemActivations(1 - progress, 0, 0);
      } else {
