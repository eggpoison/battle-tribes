...
export function createEntity(position: Point, entityType: EntityType): Entity {
   switch (entityType) {
      case EntityType.berryBush: return createBerryBush(position);
      case EntityType.boulder: return createBoulder(position);
      case EntityType.cactus: return createCactus(position);
      case EntityType.cow: return createCow(position);
      case EntityType.fish: return createFish(position);
      case EntityType.frozenYeti: return createFrozenYeti(position);
      case EntityType.iceSpikes: return createIceSpikes(position);
      ...
      case EntityType.snowball: return createSnowball(position);
      case EntityType.tombstone: return createTombstone(position);
      case EntityType.tree: return createTree(position);
      case EntityType.yeti: return createYeti(position);
      case EntityType.zombie: return createZombie(position, false, 0);
      case EntityType.spitPoison: return createSpitPoison(position);
      case EntityType.golem: return createGolem(position);
      case EntityType.pebblum: return createPebblum(position, 0);
      case EntityType.tribeWorker: return createTribeWorker(position, -1, 0);
