// @Hack: remove export
export const TEXTURE_SOURCES: Array<string> = [
   "entities/cow/cow-body-1.png",
   "entities/cow/cow-head-1.png",
   "entities/cow/cow-body-2.png",
   "entities/cow/cow-head-2.png",
   "entities/zombie/zombie1.png",
   "entities/zombie/zombie2.png",
   "entities/zombie/zombie3.png",
   "entities/zombie/zombie-golden.png",
   "entities/zombie/fist-1.png",
   "entities/zombie/fist-2.png",
   "entities/zombie/fist-3.png",
   "entities/zombie/fist-4.png",
   "entities/tombstone/tombstone1.png",
   "entities/tombstone/tombstone2.png",
   "entities/tombstone/tombstone3.png",
   "entities/tree/tree-small.png",
   "entities/tree/tree-large.png",
   "entities/workbench/workbench.png",
   "entities/plainspeople/player.png",
   "entities/plainspeople/worker.png",
   "entities/plainspeople/warrior.png",
   "entities/plainspeople/fist.png",
   "entities/plainspeople/shrewd-eye.png",
   "entities/goblins/player.png",
   "entities/goblins/worker.png",
   "entities/goblins/warrior.png",
   "entities/goblins/fist.png",
   "entities/goblins/goblin-ear.png",
   "entities/goblins/goblin-warpaint-1.png",
   "entities/goblins/goblin-warpaint-2.png",
   "entities/goblins/goblin-warpaint-3.png",
   "entities/goblins/goblin-warpaint-4.png",
   "entities/goblins/goblin-warpaint-5.png",
   "entities/goblins/warrior-warpaint-1.png",
   "entities/barbarians/player.png",
   "entities/barbarians/worker.png",
   "entities/barbarians/warrior.png",
   "entities/barbarians/fist.png",
   "entities/frostlings/player.png",
   "entities/frostlings/worker.png",
   "entities/frostlings/warrior.png",
   "entities/frostlings/fist.png",
   "entities/dwarves/player.png",
   "entities/dwarves/worker.png",
   "entities/dwarves/warrior.png",
   "entities/dwarves/fist.png",
   "scars/scar-1.png",
   "scars/scar-2.png",
   "entities/boulder/boulder1.png",
   "entities/boulder/boulder2.png",
   "entities/berry-bush1.png",
   "entities/berry-bush2.png",
   "entities/berry-bush3.png",
   "entities/berry-bush4.png",
   "entities/berry-bush5.png",
   "entities/berry-bush6.png",
   "entities/cactus/cactus.png",
   "entities/cactus/cactus-limb.png",
   "entities/cactus/cactus-flower-small-1.png",
   "entities/cactus/cactus-flower-small-2.png",
   "entities/cactus/cactus-flower-small-3.png",
   "entities/cactus/cactus-flower-small-4.png",
   "entities/cactus/cactus-flower-large-1.png",
   "entities/cactus/cactus-flower-large-2.png",
   "entities/cactus/cactus-flower-large-3.png",
   "entities/cactus/cactus-flower-large-4.png",
   "entities/cactus/cactus-flower-5.png",
   "entities/yeti/yeti.png",
   "entities/yeti/yeti-head.png",
   "entities/yeti/yeti-paw.png",
   "entities/ice-spikes/ice-spikes.png",
   "projectiles/ice-shard.png",
   "projectiles/wooden-arrow.png",
   "projectiles/wooden-bolt.png",
   "entities/snowball/size-1.png",
   "entities/snowball/size-2.png",
   "entities/snowball/size-3.png",
   "entities/snowball/size-4.png",
   "entities/slime/slime-small-body.png",
   "entities/slime/slime-medium-body.png",
   "entities/slime/slime-large-body.png",
   "entities/slime/slime-small-eye.png",
   "entities/slime/slime-medium-eye.png",
   "entities/slime/slime-large-eye.png",
   "entities/slime/slime-small-shading.png",
   "entities/slime/slime-medium-shading.png",
   "entities/slime/slime-large-shading.png",
   "entities/slime/slime-orb-small.png",
   "entities/slime/slime-orb-medium.png",
   "entities/slime/slime-orb-large.png",
   "entities/slimewisp/slimewisp.png",
   "entities/tribe-totem/tribe-totem.png",
   "entities/tribe-totem/goblin-banner.png",
   "entities/tribe-totem/barbarian-banner.png",
   "entities/tribe-totem/plainspeople-banner.png",
   "entities/tribe-totem/frostling-banner.png",
   "entities/tribe-totem/dwarf-banner.png",
   "entities/worker-hut/worker-hut.png",
   "entities/worker-hut/worker-hut-door.png",
   "entities/warrior-hut/warrior-hut.png",
   "entities/warrior-hut/warrior-hut-door.png",
   "entities/warrior-hut/warrior-hut-blueprint-1.png",
   "entities/warrior-hut/warrior-hut-blueprint-2.png",
   "entities/warrior-hut/warrior-hut-blueprint-3.png",
   "entities/warrior-hut/warrior-hut-blueprint-4.png",
   "entities/warrior-hut/warrior-hut-blueprint-5.png",
   "entities/warrior-hut/warrior-hut-blueprint-6.png",
   "entities/barrel/barrel.png",
   "entities/barrel/barrel-open.png",
   "entities/campfire/campfire.png",
   "entities/furnace/furnace.png",
   "entities/krumblid/krumblid.png",
   "entities/krumblid/mandible.png",
   // @Robustness: Hardcoded
   "miscellaneous/wooden-bow-charge-1.png",
   "miscellaneous/wooden-bow-charge-2.png",
   "miscellaneous/wooden-bow-charge-3.png",
   "miscellaneous/wooden-bow-charge-4.png",
   "miscellaneous/wooden-bow-charge-5.png",
   "miscellaneous/reinforced-bow-charge-1.png",
   "miscellaneous/reinforced-bow-charge-2.png",
   "miscellaneous/reinforced-bow-charge-3.png",
   "miscellaneous/reinforced-bow-charge-4.png",
   "miscellaneous/reinforced-bow-charge-5.png",
   "miscellaneous/ice-bow-charge-1.png",
   "miscellaneous/ice-bow-charge-2.png",
   "miscellaneous/ice-bow-charge-3.png",
   "miscellaneous/ice-bow-charge-4.png",
   "miscellaneous/ice-bow-charge-5.png",
   "miscellaneous/crossbow-charge-1.png",
   "miscellaneous/crossbow-charge-2.png",
   "miscellaneous/crossbow-charge-3.png",
   "miscellaneous/crossbow-charge-4.png",
   "miscellaneous/crossbow-charge-5.png",
   "entities/fish/fish-blue.png",
   "entities/fish/fish-gold.png",
   "entities/fish/fish-red.png",
   "entities/fish/fish-lime.png",
   "decorations/pebble.png",
   "decorations/rock1.png",
   "decorations/sandstone-rock.png",
   "decorations/sandstone-rock-big1.png",
   "decorations/sandstone-rock-big2.png",
   "decorations/sandstone-rock-dark.png",
   "decorations/sandstone-rock-dark-big1.png",
   "decorations/sandstone-rock-dark-big2.png",
   "decorations/flower1.png",
   "decorations/flower2.png",
   "decorations/flower3.png",
   "decorations/flower4.png",
   "entities/research-bench/research-bench.png",
   "entities/wall/wooden-wall.png",
   "entities/wall/wooden-wall-damage-1.png",
   "entities/wall/wooden-wall-damage-2.png",
   "entities/wall/wooden-wall-damage-3.png",
   "entities/wall/wooden-wall-damage-4.png",
   "entities/wall/wooden-wall-damage-5.png",
   "entities/wall/wooden-wall-damage-6.png",
   "entities/wall/stone-wall.png",
   "projectiles/slime-spit-medium.png",
   "projectiles/slime-spit-large.png",
   "entities/door/wooden-door.png",
   "entities/door/wooden-door-blueprint-1.png",
   "entities/door/wooden-door-blueprint-2.png",
   "entities/door/stone-door.png",
   "entities/door/stone-door-blueprint-1.png",
   "entities/door/stone-door-blueprint-2.png",
   "entities/golem/golem-body-large.png",
   "entities/golem/golem-body-medium.png",
   "entities/golem/golem-body-small.png",
   "entities/golem/golem-body-massive.png",
   "entities/golem/golem-body-tiny.png",
   "entities/golem/eye.png",
   "entities/planter-box/planter-box.png",
   "entities/plant/tree-sapling-1.png",
   "entities/plant/tree-sapling-2.png",
   "entities/plant/tree-sapling-3.png",
   "entities/plant/tree-sapling-4.png",
   "entities/plant/tree-sapling-5.png",
   "entities/plant/tree-sapling-6.png",
   "entities/plant/tree-sapling-7.png",
   "entities/plant/tree-sapling-8.png",
   "entities/plant/tree-sapling-9.png",
   "entities/plant/tree-sapling-10.png",
   "entities/plant/tree-sapling-11.png",
   "entities/plant/berry-bush-sapling-1.png",
   "entities/plant/berry-bush-sapling-2.png",
   "entities/plant/berry-bush-sapling-3.png",
   "entities/plant/berry-bush-sapling-4.png",
   "entities/plant/berry-bush-sapling-5.png",
   "entities/plant/berry-bush-sapling-6.png",
   "entities/plant/berry-bush-sapling-7.png",
   "entities/plant/berry-bush-sapling-8.png",
   "entities/plant/berry-bush-sapling-9.png",
   "entities/plant/ice-spikes-sapling-1.png",
   "entities/plant/ice-spikes-sapling-2.png",
   "entities/plant/ice-spikes-sapling-3.png",
   "entities/plant/ice-spikes-sapling-4.png",
   "entities/plant/ice-spikes-sapling-5.png",
   "entities/plant/ice-spikes-sapling-6.png",
   "entities/plant/ice-spikes-sapling-7.png",
   "entities/plant/ice-spikes-sapling-8.png",
   "entities/plant/ice-spikes-sapling-9.png",
   "entities/plant/berry-bush-plant-1.png",
   "entities/plant/berry-bush-plant-2.png",
   "entities/plant/berry-bush-plant-3.png",
   "entities/plant/berry-bush-plant-4.png",
   "entities/plant/berry-bush-plant-5.png",
   "entities/plant/dirt-clump.png",
   "entities/plant/snow-clump.png",
   "projectiles/ice-arrow.png",
   "entities/pebblum/pebblum-nose.png",
   "entities/pebblum/pebblum-body.png",
   "entities/embrasure/wooden-embrasure.png",
   "entities/embrasure/wooden-embrasure-blueprint-1.png",
   "entities/embrasure/wooden-embrasure-blueprint-2.png",
   "entities/embrasure/wooden-embrasure-blueprint-3.png",
   "entities/embrasure/stone-embrasure.png",
   "entities/embrasure/stone-embrasure-blueprint-1.png",
   "entities/embrasure/stone-embrasure-blueprint-2.png",
   "entities/embrasure/stone-embrasure-blueprint-3.png",
   "entities/spikes/wooden-floor-spikes.png",
   "entities/spikes/wooden-wall-spikes.png",
   "entities/spikes/stone-floor-spikes.png",
   "entities/spikes/stone-wall-spikes.png",
   "entities/spikes/stone-floor-spikes-blueprint-1.png",
   "entities/spikes/stone-floor-spikes-blueprint-2.png",
   "entities/spikes/stone-wall-spikes-blueprint-1.png",
   "entities/spikes/stone-wall-spikes-blueprint-2.png",
   "entities/wall-punji-sticks/wall-punji-sticks.png",
   "entities/floor-punji-sticks/floor-punji-sticks.png",
   "entities/ballista/base.png",
   "entities/ballista/shaft.png",
   "entities/ballista/crossbow.png", // @Incomplete: just for blueprints
   "entities/ballista/crossbow-1.png",
   "entities/ballista/crossbow-2.png",
   "entities/ballista/crossbow-3.png",
   "entities/ballista/crossbow-4.png",
   "entities/ballista/crossbow-5.png",
   "entities/ballista/crossbow-6.png",
   "entities/ballista/crossbow-7.png",
   "entities/ballista/crossbow-8.png",
   "entities/ballista/crossbow-9.png",
   "entities/ballista/crossbow-10.png",
   "entities/ballista/crossbow-11.png",
   "entities/ballista/gear.png",
   "entities/ballista/ammo-box.png",
   "entities/ballista/plate.png",
   "entities/ballista/ammo-warning.png",
   "entities/sling-turret/sling-turret-base.png",
   "entities/sling-turret/sling-turret-plate.png",
   "entities/sling-turret/sling-turret-sling.png",
   "entities/sling-turret/sling-charge-1.png",
   "entities/sling-turret/sling-charge-2.png",
   "entities/sling-turret/sling-charge-3.png",
   "entities/sling-turret/sling-charge-4.png",
   "entities/sling-turret/sling-charge-5.png",
   "entities/sling-turret/base-blueprint-1.png",
   "entities/sling-turret/base-blueprint-2.png",
   "entities/sling-turret/base-blueprint-3.png",
   "entities/sling-turret/base-blueprint-4.png",
   "entities/sling-turret/plate-blueprint-1.png",
   "entities/sling-turret/plate-blueprint-2.png",
   "entities/sling-turret/sling-blueprint-1.png",
   "entities/sling-turret/sling-blueprint-2.png",
   "projectiles/sling-rock.png", 
   "projectiles/ballista-rock.png",
   "projectiles/ballista-slimeball.png",
   "projectiles/ballista-frostcicle.png",
   "entities/tunnel/wooden-tunnel.png",
   "entities/tunnel/wooden-tunnel-blueprint-1.png",
   "entities/tunnel/wooden-tunnel-blueprint-2.png",
   "entities/tunnel/stone-tunnel.png",
   "entities/tunnel/stone-tunnel-blueprint-1.png",
   "entities/tunnel/stone-tunnel-blueprint-2.png",
   "entities/tunnel/tunnel-door.png",
   // @Cleanup: Not in subfolder
   "entities/low-health-marker.png",
   // @Cleanup: Not in subfolder
   "entities/deconstruct-marker.png",
   "entities/healing-totem/healing-totem.png",
   // @Cleanup: Not in subfolder
   // @Cleanup: not entity-related. would be better to be in billboads
   "entities/recall-marker.png",
   "entities/miscellaneous/bandage.png",
   "entities/miscellaneous/cover-leaf-small.png",
   "entities/miscellaneous/cover-leaf-large.png",
   "entities/miscellaneous/eye-scar.png",
   "entities/miscellaneous/tribesman-leaf.png",
   "entities/miscellaneous/tribesman-fangs.png",
   "entities/fence/fence-node.png",
   "entities/fence/fence-top-rail.png",
   "entities/fence/fence-right-rail.png",
   "entities/fence/fence-bottom-rail.png",
   "entities/fence/fence-left-rail.png",
   "entities/fence-gate/side.png",
   "entities/fence-gate/door.png",
   "entities/fence-gate/side-blueprint-1.png",
   "entities/fence-gate/door-blueprint-1.png",
   "items/large/fertiliser.png",
   "entities/frostshaper/frostshaper.png",
   "entities/stonecarving-table/stonecarving-table.png",
   "entities/miscellaneous/tribesman-health-patch.png",
   "entities/river-stepping-stone/stone-small.png",
   "entities/river-stepping-stone/stone-medium.png",
   "entities/river-stepping-stone/stone-large.png",
   "entities/lilypad/lilypad.png",
   "entities/fibre-plant/fibre-plant.png",
   "entities/shield-items/wooden-shield.png",
   "entities/guardian/guardian-head.png",
   "entities/guardian/guardian-head-rubies.png",
   "entities/guardian/guardian-body.png",
   "entities/guardian/guardian-body-amethysts.png",
   "entities/guardian/guardian-body-emeralds.png",
   "entities/guardian/guardian-limb.png",
   "entities/guardian/guardian-limb-gem-cracks.png",
   "entities/guardian-gem-quake/gem-1.png",
   "entities/guardian-gem-quake/gem-2.png",
   "entities/guardian-gem-quake/gem-3.png",
   "entities/guardian-gem-fragment-projectile/fragment-1.png",
   "entities/guardian-gem-fragment-projectile/fragment-2.png",
   "entities/guardian-gem-fragment-projectile/fragment-3.png",
   "entities/guardian-spiky-ball/guardian-spiky-ball.png",
   "entities/bracings/wooden-vertical-post.png",
   "entities/bracings/wooden-horizontal-post.png",
   "entities/bracings/stone-vertical-post.png",
   "entities/bracings/stone-horizontal-post.png",
   "entities/fire-torch/fire-torch.png",
   "entities/spiky-bastard/spiky-bastard-1.png",
   "entities/spiky-bastard/spiky-bastard-2.png",
   "entities/spiky-bastard/spiky-bastard-3.png",
   "entities/glurb/glurb-head-segment.png",
   "entities/glurb/glurb-middle-segment.png",
   "entities/glurb/glurb-tail-segment.png",
   "entities/glurb/glurb-eye.png",
   "entities/slurb-torch/slurb-torch.png",
   "entities/tree-root-base/tree-root-base.png",
   "entities/tree-root-segment/tree-root-segment-1.png",
   "entities/tree-root-segment/tree-root-segment-2.png",
   "entities/tree-root-segment/tree-root-segment-3.png",
   "entities/tree-root-segment/tree-root-segment-4.png",
   "entities/mithril-ore-node/mithril-ore-node-small-1.png",
   "entities/mithril-ore-node/mithril-ore-node-small-2.png",
   "entities/mithril-ore-node/mithril-ore-node-medium-1.png",
   "entities/mithril-ore-node/mithril-ore-node-medium-2.png",
   "entities/mithril-ore-node/mithril-ore-node-large-1.png",
   "entities/scrappy/body.png",
   "entities/scrappy/hand.png",
   "entities/cogwalker/body.png",
   "entities/cogwalker/hand.png",
   "entities/automaton-assembler/automaton-assembler.png",
   "entities/automaton-assembler/back.png",
   "entities/automaton-assembler/gear.png",
   "entities/automaton-assembler/gear-2.png",
   "entities/mithril-anvil/mithril-anvil.png",
   "entities/miscellaneous/follow-halo.png",
   "entities/miscellaneous/attack-halo.png",
   "entities/miscellaneous/carry-slot.png",
   "entities/miscellaneous/taming-tier-1.png",
   "entities/miscellaneous/taming-tier-2.png",
   "entities/miscellaneous/taming-tier-3.png",
   "entities/moss/light-green/moss-large.png",
   "entities/moss/light-green/moss-medium.png",
   "entities/moss/light-green/moss-small.png",
   "entities/moss/dark-green/moss-large.png",
   "entities/moss/dark-green/moss-medium.png",
   "entities/moss/dark-green/moss-small.png",
   "entities/moss/aqua/moss-large.png",
   "entities/moss/aqua/moss-medium.png",
   "entities/moss/aqua/moss-small.png",
   "entities/moss/red/moss-large.png",
   "entities/moss/red/moss-medium.png",
   "entities/moss/red/moss-small.png",
   "entities/moss/purple/moss-large.png",
   "entities/moss/purple/moss-medium.png",
   "entities/moss/purple/moss-small.png",
   "entities/moss/gold/moss-large.png",
   "entities/moss/gold/moss-medium.png",
   "entities/moss/gold/moss-small.png",
   "entities/glurb/moss-ball/moss-ball-1.png",
   "entities/glurb/moss-ball/moss-ball-2.png",
   "entities/glurb/moss-ball/moss-ball-3.png",
   "entities/glurb/moss-ball/moss-ball-4.png",
   "entities/glurb/moss-ball/moss-ball-5.png",
   "entities/glurb/moss-ball/moss-ball-6.png",
   "entities/floor-sign/floor-sign.png",
   "entities/desert-bush-lively/desert-bush-lively.png",
   "entities/desert-bush-sandy/desert-bush-sandy.png",
   "entities/desert-bush-sandy/desert-bush-sandy-large.png",
   "entities/desert-small-weed/desert-small-weed.png",
   "entities/desert-shrub/desert-shrub.png",
   "entities/tumbleweed-live/tumbleweed-live.png",
   "entities/tumbleweed-dead/tumbleweed-dead.png",
   "entities/palm-tree/palm-tree.png",
   "entities/prickly-pear/prickly-pear.png",
   "entities/prickly-pear-fragment-projectile/fragment-1.png",
   "entities/prickly-pear-fragment-projectile/fragment-2.png",
   "entities/dustflea/dustflea.png",
   "entities/sandstone-rock/sandstone-rock-small.png",
   "entities/sandstone-rock/sandstone-rock-medium.png",
   "entities/sandstone-rock/sandstone-rock-large.png",
   "entities/okren/juvenile/body.png",
   "entities/okren/juvenile/eye.png",
   "entities/okren/juvenile/eye-crust.png",
   "entities/okren/juvenile/mandible.png",
   "entities/okren/juvenile/big-arm-segment-1.png",
   "entities/okren/juvenile/big-arm-segment-2.png",
   "entities/okren/juvenile/big-arm-segment-3.png",
   "entities/okren/juvenile/big-arm-segment-4.png",
   "entities/okren/juvenile/medium-arm-segment-1.png",
   "entities/okren/juvenile/medium-arm-segment-2.png",
   "entities/okren/juvenile/medium-arm-segment-3.png",
   "entities/okren/juvenile/medium-arm-segment-4.png",
   "entities/okren/juvenile/arm-segment-of-slashing-and-destruction-1.png",
   "entities/okren/juvenile/arm-segment-of-slashing-and-destruction-2.png",
   "entities/okren/juvenile/arm-segment-of-slashing-and-destruction-3.png",
   "entities/okren/juvenile/arm-segment-of-slashing-and-destruction-4.png",
   "entities/okren/youth/body.png",
   "entities/okren/youth/eye.png",
   "entities/okren/youth/eye-crust.png",
   "entities/okren/youth/mandible.png",
   "entities/okren/youth/big-arm-segment-1.png",
   "entities/okren/youth/big-arm-segment-2.png",
   "entities/okren/youth/big-arm-segment-3.png",
   "entities/okren/youth/big-arm-segment-4.png",
   "entities/okren/youth/medium-arm-segment-1.png",
   "entities/okren/youth/medium-arm-segment-2.png",
   "entities/okren/youth/medium-arm-segment-3.png",
   "entities/okren/youth/medium-arm-segment-4.png",
   "entities/okren/youth/arm-segment-of-slashing-and-destruction-1.png",
   "entities/okren/youth/arm-segment-of-slashing-and-destruction-2.png",
   "entities/okren/youth/arm-segment-of-slashing-and-destruction-3.png",
   "entities/okren/youth/arm-segment-of-slashing-and-destruction-4.png",
   "entities/okren/adult/body.png",
   "entities/okren/adult/eye.png",
   "entities/okren/adult/eye-crust.png",
   "entities/okren/adult/mandible.png",
   "entities/okren/adult/big-arm-segment-1.png",
   "entities/okren/adult/big-arm-segment-2.png",
   "entities/okren/adult/big-arm-segment-3.png",
   "entities/okren/adult/big-arm-segment-4.png",
   "entities/okren/adult/medium-arm-segment-1.png",
   "entities/okren/adult/medium-arm-segment-2.png",
   "entities/okren/adult/medium-arm-segment-3.png",
   "entities/okren/adult/medium-arm-segment-4.png",
   "entities/okren/adult/arm-segment-of-slashing-and-destruction-1.png",
   "entities/okren/adult/arm-segment-of-slashing-and-destruction-2.png",
   "entities/okren/adult/arm-segment-of-slashing-and-destruction-3.png",
   "entities/okren/adult/arm-segment-of-slashing-and-destruction-4.png",
   "entities/okren/elder/body.png",
   "entities/okren/elder/eye.png",
   "entities/okren/elder/eye-crust.png",
   "entities/okren/elder/mandible.png",
   "entities/okren/elder/big-arm-segment-1.png",
   "entities/okren/elder/big-arm-segment-2.png",
   "entities/okren/elder/big-arm-segment-3.png",
   "entities/okren/elder/big-arm-segment-4.png",
   "entities/okren/elder/medium-arm-segment-1.png",
   "entities/okren/elder/medium-arm-segment-2.png",
   "entities/okren/elder/medium-arm-segment-3.png",
   "entities/okren/elder/medium-arm-segment-4.png",
   "entities/okren/elder/arm-segment-of-slashing-and-destruction-1.png",
   "entities/okren/elder/arm-segment-of-slashing-and-destruction-2.png",
   "entities/okren/elder/arm-segment-of-slashing-and-destruction-3.png",
   "entities/okren/elder/arm-segment-of-slashing-and-destruction-4.png",
   "entities/okren/ancient/body.png",
   "entities/okren/ancient/eye.png",
   "entities/okren/ancient/eye-crust.png",
   "entities/okren/ancient/mandible.png",
   "entities/okren/ancient/big-arm-segment-1.png",
   "entities/okren/ancient/big-arm-segment-2.png",
   "entities/okren/ancient/big-arm-segment-3.png",
   "entities/okren/ancient/big-arm-segment-4.png",
   "entities/okren/ancient/medium-arm-segment-1.png",
   "entities/okren/ancient/medium-arm-segment-2.png",
   "entities/okren/ancient/medium-arm-segment-3.png",
   "entities/okren/ancient/medium-arm-segment-4.png",
   "entities/okren/ancient/arm-segment-of-slashing-and-destruction-1.png",
   "entities/okren/ancient/arm-segment-of-slashing-and-destruction-2.png",
   "entities/okren/ancient/arm-segment-of-slashing-and-destruction-3.png",
   "entities/okren/ancient/arm-segment-of-slashing-and-destruction-4.png",
   "entities/okren/tongue-segment.png",
   "entities/okren/tongue-tip.png",
   "entities/dustflea-morph-cocoon/stage-1.png",
   "entities/dustflea-morph-cocoon/stage-2.png",
   "entities/dustflea-morph-cocoon/stage-3.png",
   "entities/dustflea-morph-cocoon/stage-4.png",
   "entities/sand-ball/size-1.png",
   "entities/sand-ball/size-2.png",
   "entities/sand-ball/size-3.png",
   "entities/sand-ball/size-4.png",
   "entities/sand-ball/size-5.png",
   "entities/sand-ball/size-6.png",
   "entities/krumblid-morph-cocoon/stage-1.png",
   "entities/krumblid-morph-cocoon/stage-2.png",
   "entities/krumblid-morph-cocoon/stage-3.png",
   "entities/krumblid-morph-cocoon/stage-4.png",
   "entities/krumblid-morph-cocoon/stage-5.png",
   "entities/krumblid-morph-cocoon/stage-6.png",
   "entities/krumblid-morph-cocoon/stage-7.png",
   "entities/krumblid-morph-cocoon/stage-8.png",
   "entities/krumblid-morph-cocoon/stage-9.png",
   "entities/krumblid-morph-cocoon/shitass.png",
   "entities/dustflea-egg/dustflea-egg.png",
   "entities/spruce-tree/tree-small.png",
   "entities/spruce-tree/tree-large.png",
   "entities/spruce-tree/snow-overlay-small-1.png",
   "entities/spruce-tree/snow-overlay-small-2.png",
   "entities/spruce-tree/snow-overlay-large-1.png",
   "entities/spruce-tree/snow-overlay-large-2.png",
   "entities/tundra-rock/rock-1.png",
   "entities/tundra-rock/rock-2.png",
   "entities/tundra-rock/rock-3.png",
   "entities/tundra-rock-frozen/rock-1.png",
   "entities/tundra-rock-frozen/rock-2.png",
   "entities/tundra-rock-frozen/rock-3.png",
   "entities/snowberry-bush/stage-0.png",
   "entities/snowberry-bush/stage-1.png",
   "entities/snowberry-bush/stage-2.png",
   "entities/snowberry-bush/stage-3.png",
   "entities/snobe/body.png",
   "entities/snobe/butt.png",
   "entities/snobe/ear.png",
   "entities/snobe-mound/snobe-mound.png",
   "entities/ingu-serpent/head.png",
   "entities/ingu-serpent/body-1.png",
   "entities/ingu-serpent/body-2.png",
   "entities/ingu-serpent/tail.png",
   "entities/tukmok/body.png",
   "entities/tukmok/head.png",
   "entities/tukmok/tail-segment-small.png",
   "entities/tukmok/tail-segment-medium.png",
   "entities/tukmok/tail-segment-big.png",
   "entities/tukmok-tail-club/club-segment.png",
   "entities/tukmok-trunk/middle-segment.png",
   "entities/tukmok-trunk/head-segment.png",
   "entities/tukmok-spur/spur-head.png",
   "entities/tukmok-spur/spur-shoulder-left-front.png",
   "entities/tukmok-spur/spur-shoulder-left-back.png",
   "entities/tukmok-spur/spur-shoulder-right-front.png",
   "entities/tukmok-spur/spur-shoulder-right-back.png",
   "entities/ingu-yetuksnoglurblidokowflea/body-1.png",
   "entities/ingu-yetuksnoglurblidokowflea/body-2.png",
   "entities/ingu-yetuksnoglurblidokowflea/body-3.png",
   "entities/ingu-yetuksnoglurblidokowflea/body-4.png",
   "entities/ingu-yetuksnoglurblidokowflea/dustflea-dispension-port.png",
   "entities/ingu-yetuksnoglurblidokowflea/snobe-tail.png",
   "entities/ingu-yetuk-laser/laser.png",
];

export function registerTextureSource(textureSource: string): void {
   if (!TEXTURE_SOURCES.includes(textureSource)) {
      TEXTURE_SOURCES.push(textureSource);
   }
}