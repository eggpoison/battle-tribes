...
import { ITEM_TYPE_RECORD, ITEM_INFO_RECORD, ToolItemInfo, ArmourItemInfo, Item, ConsumableItemInfo, Inventory, ItemType, HammerItemInfo, PlaceableItemInfo, ConsumableItemCategory } from "webgl-test-shared/dist/items";
...
import { distance, angle, Point, randInt } from "webgl-test-shared/dist/utils";
import Entity from "../../../Entity";
import { getEntitiesInRange, willStopAtDesiredDistance, getClosestAccessibleEntity, stopEntity, moveEntityToPosition, getAngleDiff, getDistanceFromPointToEntity } from "../../../ai-shared";
...
import { getInventory, addItemToInventory, consumeItemFromSlot, addItemToSlot, removeItemFromInventory, getItem, craftRecipe, recipeCraftingStationIsAvailable, inventoryComponentCanAffordRecipe } from "../../../components/InventoryComponent";
...
const shouldRecalculatePath = (tribesman: Entity, goalX: number, goalY: number, goalRadiusNodes: number): boolean => {
   ...
   if (tribesmanComponent.path.length === 0) {
      ...
      if (tribesmanComponent.rawPath.length > 2 && tribesman.ageTicks % Settings.TPS === 0 && tribesman.velocity.lengthSquared() < 10 * 10) {
         return true;
      }
...
const continueCurrentPath = (tribesman: Entity, goalX: number, goalY: number): boolean => {
   ...
   if (path.length > 0) {
      ...
      if (pathIsClear(tribesman.position.x, tribesman.position.y, goalX, goalY, tribeComponent.tribe.pathfindingGroupID, footprint)) {
         ...
         if (willStopAtDesiredDistance(tribesman, -2, distFromNode)) {
...
const goCraftItem = (tribesman: Entity, recipe: CraftingRecipe, tribe: Tribe): boolean => {
   ...
   if (!recipeCraftingStationIsAvailable(availableCraftingStations, recipe)) {
      ...
      if (tribesmanComponent.currentCraftingTicks >= recipe.aiCraftTimeTicks) {
         // Craft the item
         const inventoryComponent = InventoryComponentArray.getComponent(tribesman.id);
         craftRecipe(inventoryComponent, recipe, "hotbar");
...
const goPlaceBuilding = (tribesman: Entity, hotbarInventory: Inventory, tribe: Tribe, goal: TribesmanPlaceGoal): boolean => {
   ...
   if (distance < Vars.BUILDING_PLACE_DISTANCE) {
      ...
      if (distance < getTribesmanAttackRadius(tribesman)) {
         ...
         consumeItemFromSlot(inventoryComponent, "hotbar", goal.placeableItemSlot, 1);
...
const goResearchTech = (tribesman: Entity, tech: TechInfo): boolean => {
   ...
   if (occupiedBenchID !== 0) {
      ...
      tribesman.acceleration.x = slowAcceleration * Math.sin(targetDirection);
      tribesman.acceleration.y = slowAcceleration * Math.cos(targetDirection);
...
export function tickTribesman(tribesman: Entity): void {
   ...
   for (const entity of aiHelperComponent.visibleEntities) {
      ...
      if (playerComponent.interactingEntityID === tribesman.id) {
         ...
         tribesman.turn(tribesman.position.calculateAngleBetween(entity.position), TRIBESMAN_TURN_SPEED);
         const distance = tribesman.position.calculateDistanceBetween(entity.position);
   ...
   if (hammerItemSlot !== 0) {
      ...
      if (typeof closestBlueprint !== "undefined") {
         ...
         if (willStopAtDesiredDistance(tribesman, desiredAttackRange - 20, distance)) {
            ...
            tribesman.acceleration.x = getTribesmanSlowAcceleration(tribesman) * Math.sin(tribesman.rotation + Math.PI);
            tribesman.acceleration.y = getTribesmanSlowAcceleration(tribesman) * Math.cos(tribesman.rotation + Math.PI);
         } else if (willStopAtDesiredDistance(tribesman, desiredAttackRange, distance)) {
            stopEntity(tribesman);
         } else {
   ...
   if (recruitTarget !== null) {
      ...
      if (typeof relation !== "undefined" && relation >= 50) {
         ...
         if (giftItemSlot !== 0) {
            ...
            const giftRange = 50;
            if (willStopAtDesiredDistance(tribesman, giftRange, distance)) {
               if (willStopAtDesiredDistance(tribesman, giftRange - 20, distance)) {
                  tribesman.acceleration.x = getTribesmanSlowAcceleration(tribesman) * Math.sin(tribesman.rotation + Math.PI);
                  tribesman.acceleration.y = getTribesmanSlowAcceleration(tribesman) * Math.cos(tribesman.rotation + Math.PI);
               } else {
                  stopEntity(tribesman);
               ...
               if (Math.abs(getAngleDiff(targetDirection, tribesman.rotation)) < 0.1 && !itemThrowIsOnCooldown(tribesmanComponent)) {
                  ...
                  throwItem(tribesman, "hotbar", giftItemSlot, item.count, targetDirection);
               }
   ...
   if (goal !== null) {
      ...
      switch (goal.type) {
         case TribesmanGoalType.craftRecipe: {
            if (inventoryComponentCanAffordRecipe(inventoryComponent, goal.recipe, "hotbar")) {
   ...
   if (goals.length >= 2 && goal!.type === TribesmanGoalType.gatherItems && goals[1].type === TribesmanGoalType.researchTech) {
      ...
      for (let itemSlot = 1; itemSlot <= hotbarInventory.width * hotbarInventory.height; itemSlot++) {
         if (hotbarInventory.itemSlots[itemSlot] === undefined) {
            continue;
         }
         ...
         consumeItemFromSlot(inventoryComponent, "hotbar", itemSlot, amountUsed);
      }
...
const escape = (tribesman: Entity, visibleEnemies: ReadonlyArray<Entity>, visibleHostileMobs: ReadonlyArray<Entity>): void => {
   ...
   tribesman.turn(runDirection, TRIBESMAN_TURN_SPEED);
