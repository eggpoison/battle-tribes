...
import { Point, customTickIntervalHasPassed, lerp, randAngle, randFloat, randInt, randItem, rotateXAroundOrigin, rotateYAroundOrigin, secondsToTicks } from "webgl-test-shared/src/utils";
import { Entity, LimbAction } from "webgl-test-shared/src/entities";
...
import { getItemRecipe } from "webgl-test-shared/src/items/crafting-recipes";
import { ItemType, ITEM_INFO_RECORD, ConsumableItemInfo } from "webgl-test-shared/src/items/items";
...
export function createCraftingAnimationParticles(entity: Entity, limbIdx: number): void {
   ...
   for (const itemTypeString of Object.keys(recipe.ingredients)) {
      ...
      if (Math.random() >= 2.5 * Settings.DT_S) {
         continue;
      }
