...
export function openCraftingMenu(): void {
   ...
   const recipeBrowserHeight = Math.max(Vars.RECIPE_BROWSER_MIN_HEIGHT, Math.ceil(availableRecipes.length / Vars.RECIPE_BROWSER_WIDTH));

   const recipeBrowserItemSlots = createInventoryContainer(false, Vars.RECIPE_BROWSER_WIDTH);
   recipeBrowserElem.appendChild(recipeBrowserItemSlots);

   const recipeBrowserNumItemSlots = recipeBrowserHeight * Vars.RECIPE_BROWSER_WIDTH;
   // Recipe slots
